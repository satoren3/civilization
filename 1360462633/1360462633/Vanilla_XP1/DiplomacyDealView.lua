		...
		for i, entry in ipairs(possibleResources) do
			if (entry.Duration == 0) then
				...
				if (not ms_bIsDemand) then
					...
					--ARISTOS:
					icon.TradeableIndicator:SetHide(true);
					--
...
function PopulateAvailableResources(player : table, iconList : table, className : string)
	...
	--ARISTOS:
	local pLocalPlayerResources = ms_LocalPlayer:GetResources();
	--
		...
		for i, entry in ipairs(possibleResources) do
			...
			if (resourceDesc ~= nil) then
				...
				if (entry.MaxAmount > 0 and resourceDesc.ResourceClassType == className ) then
					...
					--ARISTOS:
					if (player:GetID() ~= ms_LocalPlayer:GetID() or GetOtherPlayer(player):GetID() ~= ms_LocalPlayer:GetID()) then
						icon.TradeableIndicator:SetHide(pLocalPlayerResources:HasResource(entry.ForType));
					end
					--
